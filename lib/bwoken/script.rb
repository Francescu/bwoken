...
module Bwoken
  ...
  class Script
    ...
    def cmd
      ...
      %Q|instruments -t Automation -D "#{self.class.trace_file_path}" -w "#{device_flag}" "#{app_dir}" "#{env_variables_for_cli}"|.tap{|x| puts "SCRIPT: #{x}"}
    end
    ...
    def device_flag
      ...
      simulator ? 'iPhone 5s (8.1 Simulator)' : "#{Bwoken::Device.uuid}"
    end
